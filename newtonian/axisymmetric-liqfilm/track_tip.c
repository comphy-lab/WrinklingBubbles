...
float Oh, k;
double x_l = Xcent; //height of the liquid layer
...
int main(int a, char const *arguments[]){
  ...
  foreach(){
    if ((f[] > f_thresh) && (f[] < 1. - f_thresh) && (d[] == MainPhase)){
      ...
      if (facets (n1, alpha1, segment1) == 2){
        ...
        if ((xp>x_l)&&(xp>xMax)){//check max film height 
          xMax = xp;
          x1 = xp;
          y1 = yp;        
        }
        if ((xp>x_l+0.1)&&(yp<yMin)){//track innermost point-film tip
          yMin = yp;
          y2 = yp;
          x2 = xp;
  ...
  foreach(){
    if ((flag != 1)&&(f[] > f_thresh) && (f[] < 1. - f_thresh) && (d[] == MainPhase)){
      ...
      if (facets (n1, alpha1, segment1) == 2){
        ...
        if ((xp1>x_l+0.1)&&(xp1>x2)&&(xp1<x1)&&(yp1>yMax)){
          yMax = yp1;
          x3 = xp1;
          y3 = yp1;
        }
  ...
  if ((flag != 1)&&(x2<x_t)){//flipped film
    ...
    // if ((y_tMax-y_tMin)>(2/k)){
    //   fprintf(ferr, "film tip reached the bottom -3-incorrect tip");
    //   return 1;
    // }
    y_av = 0.5*(y_tMin+y_tMax);
  ...
  //return 1;
  //recentre the x-coordinate to the liq film height
  x1 = x1-x_l;
  x2 = x2-x_l;
  x3 = x3-x_l;
  x_tip = x_tip - x_l;
